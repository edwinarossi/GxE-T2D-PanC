...
```{r T2D and Age Interaction on PanC Risk, echo = FALSE}
...
# ORs and CIs
or_CI_RG_panc_main_covs <- round(exp(cbind(coef(RG_pred_panc_out_cov),
                    confint(RG_pred_panc_out_cov))),  digits = 2) %>% as.data.frame()
...
t2d_prs_panc_RG = glm(PANC.incident.RG ~ score_norm + Sex + AgeBaseline + Fasting.time +
                        Fasting.time^2 + Fasting.time^3,
                   family=binomial("logit"), 
                   data = t2d_prs_with_pheno)
...
t2d_prs_panc_RG_gxe = glm(PANC.incident.RG ~ score_norm + RG.control.log +
                            score_norm*RG.control.log* + Sex + AgeBaseline + BMI + Fasting.time +
                            Fasting.time^2 + Fasting.time^3 + 
                            Array + PC1 + PC2 + PC3 + PC4 + PC5 + PC6,
                       family=binomial("logit"), data = t2d_prs_with_pheno)
...
plot_logit_model <- or_CI_t2d_PRS_RG[-c(1, 8, 9, 10, 11, 12, 13, 14),] %>%  #remove the rows we don't wanna plot
  ggplot(aes(x = reorder(variable, AOR), y = AOR)) +
  geom_point(shape = 16,
             size  = 3.5, width = 0.1,
             position = "dodge", color="black") + 
...
panc_prs_t2d_interact_plot +
  ...
  theme(legend.key.size = unit(0.8, 'cm'), plot.title = element_text(face = "bold", 
                                                                     size = 12)) + 
  ggtitle("Interaction Plot of PanC PRS and T2D Status") + 
