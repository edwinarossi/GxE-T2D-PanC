...
epi_data <- fread("~/Desktop/Masters Project/GxE Testing PanC/RG-PanC-2022-05-28.csv")
...
#Chi-squared test of association between T2D and PanC 
...
c25_sum = sum(epi_data$C25 ==1)
t2d_sum = sum(epi_data$T2D ==1)
t2_c25_1year_sum = sum(epi_data$T2D.PANC.1year)
...
# What are normalized levels? Research this and check with Mila! 
```
...
# Mean and SD of RG measurements of participants with a PANC diagnosis and no T2D diagnosis
mean_RG_level_PANC_noT2D = mean(subset(epi_data, epi_data$C25 == 1 & 
                                 epi_data$T2D == 0)$Random.glucose, na.rm = T)
SD_RG_level_PANC_noT2D = sd(subset(epi_data, epi_data$C25 == 1 & 
                                 epi_data$T2D == 0)$Random.glucose, na.rm = T)

# saving the phenotypic file I have used herein, so that I can use it in my regression analyses 
write.table(epi_data, "~/Desktop/Masters Project/GxE Testing PanC/Pheno_file_RG_T2D_PANC.txt", row.names = F)
